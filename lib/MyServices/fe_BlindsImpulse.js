      ...
      case "TargetPosition":
        newService
          ...
          .on("set", (value, cb) => {
            ...
            newService.setCharacteristic(Characteristic.PositionState, moveUp ? 1 : 0);

            if (moveUp) {
              Bobaos.setValue({id: c.open, value: 1}).then(_ => {
                setTimeout(_ => {
                  Bobaos.setValue({id: c.open, value: 0});
                }, 1000);
              }).catch(e => {
                log(e);
              });
            } else {
              Bobaos.setValue({id: c.close, value: 1}).then(_ => {
                setTimeout(_ => {
                  Bobaos.setValue({id: c.close, value: 0});
                }, 1000);
              }).catch(e => {
                log(e);
              });
            ...
            interval = setInterval(function () {
              ...
              if (lastPosition === currentTargetPosition) {
                if (currentTargetPosition !== 0 && currentTargetPosition !== 100) {
                  Bobaos.setValue({id: c.stop, value: 1})
                    .then(_ => {
                      newService.setCharacteristic(Characteristic.CurrentPosition, value);
                      newService.setCharacteristic(Characteristic.PositionState, 2);
                      lastPosition = value;
                      setTimeout(_ => {
                        Bobaos.setValue({id: c.stop, value: 0});
                      }, 1000);
                    })
                    .catch(e => {
                      log(e);
